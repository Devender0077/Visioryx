...
- ✅ **Server.py modular split (06-23 — phase 2 complete)** — extracted shared deps (`deps.py`) + Pydantic models (`schemas.py`) + 7 router files (`routers/{auth,users,analytics,cameras,alerts,detections,settings}.py`). server.py dropped from 1117 → ~595 lines. Remaining in server.py: lifespan/seed/demo broadcast loop/ws/enroll/exception-handler/AI router mount. All 12 endpoints (auth/me, users, cameras, alerts, detections, audit, analytics/{overview,detection-trends,detection-status-trends,object-stats}, settings/email, stream/status) verified 200 OK.
- ✅ **Real detection-status persistence (06-23)** — `/analytics/detection-status-trends` and `/analytics/object-stats` now compute real known/unknown splits + per-object-class counts from `db.alerts` via Mongo aggregation pipelines (`$dateToString` group + `$regexMatch` heuristic). Random-mock removed.
