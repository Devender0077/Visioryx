...
- ✅ **Audit Log dashboard (06-23)** — `/audit` screen redesigned: color-coded action chips per type, actor-email substring filter, action-name chip filter, CSV export (web only), formatted IP/actor/detail JSON inline with each row. Reads from real persisted `audit_logs`.
- ✅ **Server.py modular split (06-23 — partial)** — extracted shared deps (`deps.py`: db, security, audit, current_user, require_admin) + Pydantic models (`schemas.py`) + auth router (`routers/auth.py`: login, register, forgot, change, me) + users router (`routers/users.py`: list/create/patch/delete + enrollment-link). server.py shrank from 1117 → ~960 lines. Future: extract analytics, settings, cameras, alerts, detections, ws.
- ✅ **Native light-mode (06-23 — primitives)** — shared vx primitives (`SectionEyebrow`, `ScreenTitle`, `ScreenSub`, `VxCard`, `VxButton`, `VxInput`, `ErrorBanner`) now consume `useColors()` at render time and override StyleSheet colors inline. Native iOS/Android screens that go through these primitives now respond to the light/dark toggle. Custom screens with their own StyleSheets remain dark on native — full coverage requires per-screen pass.
